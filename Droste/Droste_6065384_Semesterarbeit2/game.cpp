...
void Game::speichern(){
    ...
    if (fileName.isNull() == false) {
        ...
        //Rückmeldung über die Speicherung des Spiels
        else if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
            QMessageBox::information(this, tr("Bravo"),
                tr("Dein Spiel wird gespeichert"), QMessageBox::Ok);
        }
...
void Game::load()
{
    ...
    //Rückmeldung, dass der Spielstand geladen wird
    else if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        QMessageBox::information(this, tr("Fantastisch"),
            tr("Dein Spielstand wird wieder hergestellt"), QMessageBox::Ok);
    }
